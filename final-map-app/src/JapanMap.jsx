import React from 'react';

// SVGファイルを「生のテキスト」として読み込むために「?raw」をつけるのがポイント！
import JapanSvg from './assets/japan_map.svg?raw';

function JapanMap() {
  const handlePrefectureClick = (event) => {
    // クリックされた要素のIDを取得
    const prefectureId = event.target.id;

    // IDがあった場合のみ処理する
    if (prefectureId) {
      // IDを都道府県名に変換する
      const prefectureName = getIdToName(prefectureId); 
      
      // アラートで表示
      alert(`${prefectureName} がクリックされたよ！`);
      
      // TODO: ここで投稿画面を開くなどの次の処理を書く
    }
  };

  return (
    <div style={{ width: '100%', height: 'auto' }}>
      {/* SVGの文字列をHTMLとして埋め込むための特殊な書き方。
        クリックイベントをこのdivに設定する。
      */}
      <div
        dangerouslySetInnerHTML={{ __html: JapanSvg }}
        style={{ width: '100%', height: 'auto', cursor: 'pointer' }}
        onClick={handlePrefectureClick}
      />
    </div>
  );
}

// 都道府県IDと名前を対応させるための関数（例）
// SVGファイルによってIDの形式は違うから、自分のSVGに合わせて調整してね！
const getIdToName = (id) => {
  const idMap = {
    'JP-01': '北海道', 'JP-02': '青森県', 'JP-03': '岩手県', 'JP-04': '宮城県',
    'JP-05': '秋田県', 'JP-06': '山形県', 'JP-07': '福島県', 'JP-08': '茨城県',
    'JP-09': '栃木県', 'JP-10': '群馬県', 'JP-11': '埼玉県', 'JP-12': '千葉県',
    'JP-13': '東京都', 'JP-14': '神奈川県','JP-15': '新潟県', 'JP-16': '富山県',
    'JP-17': '石川県', 'JP-18': '福井県', 'JP-19': '山梨県', 'JP-20': '長野県',
    'JP-21': '岐阜県', 'JP-22': '静岡県', 'JP-23': '愛知県', 'JP-24': '三重県',
    'JP-25': '滋賀県', 'JP-26': '京都府', 'JP-27': '大阪府', 'JP-28': '兵庫県',
    'JP-29': '奈良県', 'JP-30': '和歌山県','JP-31': '鳥取県', 'JP-32': '島根県',
    'JP-33': '岡山県', 'JP-34': '広島県', 'JP-35': '山口県', 'JP-36': '徳島県',
    'JP-37': '香川県', 'JP-38': '愛媛県', 'JP-39': '高知県', 'JP-40': '福岡県',
    'JP-41': '佐賀県', 'JP-42': '長崎県', 'JP-43': '熊本県', 'JP-44': '大分県',
    'JP-45': '宮崎県', 'JP-46': '鹿児島県','JP-47': '沖縄県'
  };
  return idMap[id] || '不明な地域';
};

export default JapanMap;