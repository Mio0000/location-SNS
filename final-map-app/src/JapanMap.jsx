  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {imagePositions.map(pos => (
        <img key={pos.id} src={`http://localhost:3001/uploads/${pos.image}`} alt={pos.text} onClick={() => onPinClick(pos)} style={{ position: 'absolute', top: `${pos.y}px`, left: `${pos.x}px`, width: '50px', height: '50px', objectFit: 'cover', borderRadius: '50%', border: '2px solid white', boxShadow: '0 0 5px rgba(0,0,0,0.5)', transform: 'translate(-50%, -50%)', zIndex: 10, cursor: 'pointer' }} />
      ))}
    </div>
  );
}
